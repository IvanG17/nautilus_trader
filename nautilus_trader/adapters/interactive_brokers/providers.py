...
class InteractiveBrokersInstrumentProvider(InstrumentProvider):
    ...
    async def get_contract_details(
        ...
        if contract.build_options_chain or contract.build_futures_chain:
            underlying_con_id = await self.get_underlying_con_id(contract)
            if underlying_con_id is None:
                self._log.warning(
                    f"No conId found for {contract.symbol} and {contract.exchange}",
                )
            else:
                contract_dict = contract.dict()
                contract_dict["conId"] = underlying_con_id
                contract = IBContract(**contract_dict)
    ...
    async def get_underlying_con_id(
        self,
        underlying: IBContract,
    ) -> int | None:
        base_underlying = IBContract(
            secType=underlying.secType,
            symbol=underlying.symbol,
            exchange=underlying.exchange,
            currency=underlying.currency,
        )
        base_instrument = await self.get_instrument(base_underlying)
        if base_instrument is None:
            return None

        return base_instrument.info["contract"]["conId"]
