#!/usr/bin/env python3
# -------------------------------------------------------------------------------------------------
#  ASTS SuperStrat Optimization Script
# -------------------------------------------------------------------------------------------------

"""
ASTS SuperStrat Optimization

Finds optimal parameters for the SuperStrat strategy (Supertrend + VSA + Donchian + Pyramiding)
across multiple timeframes using Bayesian optimization with walk-forward validation.

SuperStrat enhancements over Flexible Entry:
...
Output:
    results/asts_superstrat_optimization/
...
# SuperStrat parameter search space (WIDE)
PARAM_SPACE = {
    # Base Flexible Entry params - WIDE ranges
    "atr_period": ("int", 5, 30),                  # ATR calculation period
    "atr_multiplier": ("float", 1.0, 5.0),         # Supertrend band multiplier
    "vsa_window": ("int", 5, 50),                  # VSA rolling window
    "vsa_volume_factor": ("float", 1.0, 3.0),      # High volume threshold
    "trailing_stop_atr_mult": ("float", 0.5, 4.0), # Trailing stop ATR multiplier
    "position_size_pct": ("float", 0.10, 0.50),    # Position size %

    # Donchian breakout params - WIDE ranges
    "donchian_period": ("int", 5, 50),             # N-period high lookback

    # Pyramiding params - WIDE ranges
    "pyramid_atr_threshold": ("float", 0.5, 3.0),  # ATR profit to trigger add
    "pyramid_size_pct": ("float", 0.2, 1.0),       # % of original size to add
}

# Output directory
OUTPUT_DIR = PROJECT_ROOT / "results" / "asts_superstrat_optimization"
...
def main():
    """Run ASTS SuperStrat optimization."""
    print("=" * 70)
    print("  ASTS SuperStrat Optimization")
    print("  (Supertrend + VSA + Donchian Breakout + Pyramiding)")
    print("=" * 70)
    print()
    ...
    for timeframe in TIMEFRAMES:
        ...
        try:
            result = optimizer.optimize(
                strategy_class=SuperStratStrategy,
                strategy_config_class=SuperStratConfig,
                bars=bars,
    ...
    print(f"| {'Timeframe':^9} | {'Sharpe':^8} | {'Best Params':^60} |")
    print(f"|{'-' * 11}|{'-' * 10}|{'-' * 62}|")
    ...
    for tf, result in sorted(best_by_timeframe.items(), key=lambda x: -x[1].best_sharpe):
        params_str = ", ".join(f"{k}={v:.2f}" if isinstance(v, float) else f"{k}={v}"
                              for k, v in list(result.best_params.items())[:5])
        print(f"| {tf:^9} | {result.best_sharpe:^8.3f} | {params_str:^60} |")
...
def save_results(
    ...
    with open(txt_path, "w") as f:
        f.write("ASTS SuperStrat Optimization Summary\n")
        f.write("=" * 50 + "\n")
        f.write(f"Date: {datetime.now().isoformat()}\n")
        f.write(f"Symbol: {SYMBOL}\n")
        f.write(f"Strategy: SuperStrat (Supertrend + VSA + Donchian + Pyramiding)\n")
        f.write(f"Trials per timeframe: {N_TRIALS}\n")
        f.write(f"Walk-forward folds: {N_SPLITS}\n\n")

        f.write("Enhancements over Flexible Entry:\n")
        f.write("  1. Donchian Breakout - Entry on N-period high break\n")
        f.write("  2. Pyramiding - Add 50% at +1.5 ATR profit\n\n")
