#!/usr/bin/env python3
# -------------------------------------------------------------------------------------------------
#  ASTS Kinetic Trend Optimization Script
# -------------------------------------------------------------------------------------------------

"""
ASTS Kinetic Trend Optimization

Finds optimal parameters for the Kinetic Trend strategy across multiple timeframes
using Bayesian optimization with walk-forward validation.

Usage:
    cd /Users/iguan/Projects/nautilus_trader
    source .venv/bin/activate
    python scripts/optimize_asts.py

Output:
    results/asts_optimization/
...
# CRITICAL: Filter sys.path to avoid local nautilus_trader source conflicting with installed package
# Keep venv site-packages but exclude the project's nautilus_trader/ directory
filtered_path = []
for p in sys.path:
    # Skip the local nautilus_trader source directory
...
# Kinetic Trend parameter search space
PARAM_SPACE = {
    "bb_period": ("int", 10, 30),           # Bollinger Bands period
    "bb_std": ("float", 1.5, 3.0),          # Bollinger Bands std dev
    "max_pyramid_levels": ("int", 1, 5),    # Maximum pyramid additions
    "pyramid_threshold": ("float", 0.01, 0.05),  # Price move % to pyramid
    "position_size_pct": ("float", 0.10, 0.30),  # Position size as % of cash
    "profit_target_pct": ("float", 0.05, 0.20),  # Take profit %
}

# Output directory
OUTPUT_DIR = PROJECT_ROOT / "results" / "asts_optimization"
...
def main():
    """Run ASTS optimization."""
    print("=" * 70)
    print("  ASTS Kinetic Trend Optimization")
    print("=" * 70)
    print()
    ...
    print(f"| {'Timeframe':^9} | {'Sharpe':^8} | {'Best Params':^40} |")
    print(f"|{'-' * 11}|{'-' * 10}|{'-' * 42}|")
    ...
    for tf, result in sorted(best_by_timeframe.items(), key=lambda x: -x[1].best_sharpe):
        params_str = ", ".join(f"{k}={v:.2f}" if isinstance(v, float) else f"{k}={v}"
                              for k, v in list(result.best_params.items())[:3])
        print(f"| {tf:^9} | {result.best_sharpe:^8.3f} | {params_str:^40} |")
...
def save_results(
    ...
    with open(txt_path, "w") as f:
        f.write("ASTS Kinetic Trend Optimization Summary\n")
        f.write("=" * 50 + "\n")
